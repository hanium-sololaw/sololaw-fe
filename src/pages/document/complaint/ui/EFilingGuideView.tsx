...
export default function EFilingGuideView({
  doc,
  form,
  typeTitle,
  onEdit,
  onBack,
}: EFilingGuideViewProps) {
  const claimValue = Number(form.objectValue) || Number(form.claimAmount) || 0;
  const plaintiffCount = form.plaintiffs.length || 1;
  const defendantCount = form.defendants.length || 1;

  const [electronicCost, setElectronicCost] =
    useState<LitigationCostResult | null>(null);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        ...
        <p className="mt-4 rounded-xl bg-gray-50 p-3.5 text-sm leading-relaxed text-gray-600">
          ...
          법원이 부여하는 번호라 저희는 조회할 수 없어요. 접수하고 나면
          전자소송은 「나의전자소송」에서, 종이 제출은 접수증에서 확인해 여기
          적어 주세요.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_320px]">
        ...
        <div className="flex flex-col gap-5">
          ...
          <p className="text-xs leading-relaxed text-gray-400">
            나홀로법에는 소장을 접수하거나 비용을 받지 않습니다. 접수와 납부는
            법원 또는 전자소송포털에서 직접 하셔야 해요.
          </p>
        </div>
      </div>
